...
class cis_config::darwin::section2::s2_1_1 {
  exec { 'bluetooth-device-check':
    ...
    unless  => ['/usr/sbin/system_profiler SPBluetoothDataType | grep "Connected: Yes"']['/usr/bin/defaults write /Library/Preferences/com.apple.Bluetooth ControllerPowerState | grep -q "0"'],
